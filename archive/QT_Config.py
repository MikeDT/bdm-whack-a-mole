...
class QT_Basic(QtWidgets.QMainWindow):
    def __init__(self, ui_file_loc='ui\\QT_Config.ui', *args, **kwargs):
    ...
    def get_save_checks(self, check_list=[True]):
        save = True
        error_list = []
        for check in check_list:
            error, check_res = check()
            save *= check_res
            error_list += [error]
        return save, error_list
    
    def save(self, save_object, file_type='pkl', filename='save_file'):
        if file_type == 'pkl':
            self.save_as_pkl(save_object, filename)
        elif file_type == 'csv':
            if type(save_object) == list:
                self.save_as_pkl(save_object, filename)
                print('Save object pickled as non list object was passed')
                      
    def save_as_pkl(self, save_object, filename):
        pkl = open(filename + '.pkl', 'wb')
        pickle.dump(save_object, pkl)
        pkl.close()

    def save_as_csv(self, save_object, filename):
        pkl = open(filename + '.pkl', 'wb')
        pickle.dump(save_object, pkl)
        pkl.close()
...
class QT_Config(QT_Basic):
    ...
    def __init__(self, *args, **kwargs):
        QT_Basic.__init__(self, ui_file_loc='ui\\QT_Config.ui', *args, **kwargs)
    ...
    def save_button_clicked(self):
        can_save, error_list = self.save_check
        if can_save:
            self.save(self.save_info, 'pkl', self.save_filename)
        else:
            self.write_error(error_list)

class QT_GUI(QT_Basic):

    def __init__(self, *args, **kwargs):
        QT_Basic.__init__(self, ui_file_loc='ui\\QT_Screen.ui', *args, **kwargs)
