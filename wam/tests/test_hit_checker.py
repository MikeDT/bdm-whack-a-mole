...
def test_check_mole_hit():
    test_hit_checker = hit_checker.Hit_Checker(10, 'Standard', luck_mean=0,
                                               luck_sd=0.05, luck_low_bnd=-0.1,
                                               luck_high_bnd=0.1, diff_fact=1)
    assert test_hit_checker.check_mole_hit(2, 0, 25, 10) == [False, False, False]
    assert test_hit_checker.check_mole_hit(2, 0, 10, 10) == [True, True, True]
    assert test_hit_checker.check_mole_hit(2, 0, 12, 10) == [False, True, True]
